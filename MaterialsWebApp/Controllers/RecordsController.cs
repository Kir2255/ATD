...
namespace MaterialsWebApp.Controllers
{
    public class RecordsController : Controller
    {
        ...
        public async Task<IActionResult> Index(string name, int page = 1, Sort sortOrder = Sort.IdAsc)
        {
            ...
            List<IncomeExpenseBook> items = context.IncomeExpenseBook.Include(r => r.Component.Detail).Include(r => r.Component.Material).Include(r => r.Component.Unit).ToList();
            if (!String.IsNullOrEmpty(name))
            {
                items = items.Where(r => r.Component.Material.Name.Contains(name)).ToList();
            }
